...
export const CheckinConfirmModal = () => {
  ...
  const checkinType = activeEmployee.lastCheckin
    ? activeEmployee.lastCheckin.check_out
      ? CHECK_IN
      : CHECK_OUT
    : CHECK_IN
  const lastCheckinId = activeEmployee.lastCheckin ? activeEmployee.lastCheckin.id : null
  const handleClose = () => {
    if (!processing) dispatch(setActiveEmployee(null))
  }
  const handleCheckin = () => {
    ...
    setProcessing(true)
    axios
      .post(`/api/employees/${activeEmployee.id}/checkin`, { employee: activeEmployee.id, checkin: lastCheckinId })
      .then((response) => {
                  ...
                  {!error && (
                    <div className='mt-3 sm:mt-5'>
                      ...
                      <div className='text-md mt-4 text-left'>
                        ...
                        <div className='text-gray-500'>U toku istog dana mozete se vise puta prijaviti i odjaviti.</div>
                      </div>
                    </div>
                  )}
                  {!!error && (
